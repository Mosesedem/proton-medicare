...
async function getUpcomingRenewals(userId: string) {
  ...
  // Map database renewals to include benefits from constants.ts
...
export default async function NigerianInsuranceDashboard() {
  ...
  const planBenefits = Array.from(
    new Set(
      healthPlans.flatMap((plan) =>
        plan.additionalBenefits.map((benefit) => ({
          benefit,
          coverage: "Included", // Since your data doesn't specify coverage amounts
        })),
      ),
    ),
  ).slice(0, 6);
                  ...
                  <tbody>
                    {healthPlans.map((plan) => (
                      <tr key={plan.id} className="border-b last:border-0">
                        ...
                        <td className="py-4 text-sm">HMO</td>
                  ...
                  >
                    <div className="mb-3 md:mb-0">
                      ...
                      <h3 className="mt-1 font-medium">
                        {renewal.Plan[0]?.name || "Unknown Plan"}
                      </h3>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-lg font-bold text-primary">
                        {formatCurrency(renewal.amount ?? 0)}
                      </div>
